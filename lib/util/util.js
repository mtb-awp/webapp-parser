 ...
 */
exports.genComboScript = function (content) {
    ...
    if (urls.length) {
        ...
        urls.forEach(function (url) {
            var url = (url.replace("http://a.tbcdn.cn/g", "http://g.tbcdn.cn"));

            if (url.indexOf("http://g.tbcdn.cn/") == 0) {
                arr.push(url.replace("http://g.tbcdn.cn/", ""));
            } else {
                throw("can't combo : " + url);
            }
        });

        var comboUrl = "http://g.tbcdn.cn/??" + arr.join(",")
